...
namespace Asset_Tender_BackEnd.Services
{
    public class EmailService : IEmailService
    {
        ...
        public async Task SendInvoiceAsync(string toEmail, string subject, string htmlBody, byte[]? attachmentBytes = null, string? attachmentName = null)
        {
            await SendHtmlEmailAsync(toEmail, subject, htmlBody, attachmentBytes, attachmentName);
        }

        private async Task SendHtmlEmailAsync(string toEmail, string subject, string htmlBody, byte[]? attachmentBytes = null, string? attachmentName = null)
        {
            ...
            // Attach directly from the byte array using a MemoryStream
            if (attachmentBytes != null && attachmentBytes.Length > 0)
            {
                var memoryStream = new MemoryStream(attachmentBytes);
                var name = !string.IsNullOrEmpty(attachmentName) ? attachmentName : "Invoice.pdf";
                var attachment = new Attachment(memoryStream, name, "application/pdf");
